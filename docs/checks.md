...
Both git calls above pass `--no-optional-locks`. Without it, git refreshes
...
run. The boot path reads it and does nothing else.
...
anywhere is not shown. The seven ids a screen can phrase are `boot_order`,
`boot_dirty`, `irq_conflict`, `virus_one`, `virus_many`, `f1` and
`f1_resume`. The `unicorn` flavour deliberately ships no `[findings]` table
of its own and inherits the machine's wording unchanged.
